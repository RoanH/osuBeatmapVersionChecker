...
{	  	
	 ...
	 *			more frames. 
	 */
	public boolean play(int frames, float volume) throws JavaLayerException
	{
		boolean ret = true;
			
		while (frames-- > 0 && ret)
		{
			ret = decodeFrame(volume);			
		}
	 ...
	 */
	protected boolean decodeFrame(float volume) throws JavaLayerException
	{		
		try
		{
			AudioDevice out = audio;
			...
			synchronized (this)
			{
				...
				if (out!=null)
				{					
					out.write(output.getBuffer(), 0, output.getBufferLength(), volume);
				}				
			}
